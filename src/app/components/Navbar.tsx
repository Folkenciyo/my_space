...
export function Navbar() {
  ...
  const icons = [
    {
      href: "#home",
      icon: <HomeIcon className="h-5 w-5 text-cyan-400" />,
      id: "home",
    },
    {
      href: "#about",
    ...
    {
      href: "#skills",
      icon: <SkillsIcon className="h-5 w-5 text-cyan-400" />,
      id: "skills",
    },
    {
      href: "#projects",
      icon: <FolderIcon className="h-5 w-5 text-cyan-400" />,
      id: "projects",
    },
    {
      href: "#contact",
      icon: <ChatIcon className="h-5 w-5 text-cyan-400" />,
      id: "contact",
    },
  ];

  return (
    <nav className="fixed left-4 top-1/2 -translate-y-1/2 backdrop-blur-sm bg-white/10 shadow-lg shadow-black/10 rounded-full p-3 z-40">
      <div className="relative flex flex-col gap-4 items-center">
        {icons.map(({ href, icon, id }) => (
          <a
            key={id}
            href={href}
            className="p-2 hover:bg-white/5 rounded-full transition-all relative"
          >
            {icon}
            {activeSection === id && (
              <motion.div
